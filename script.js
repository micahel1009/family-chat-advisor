...
async function sendMessage() {
    ...
    // 修正：加入多樣化的互動挑戰清單，要求 AI 根據情境隨機選擇一個。
    ...
    2. **如果對話次數大於等於 3 (轉折與大冒險)：**
       ...
       - **段落 2 (提出大冒險)：** 說明現在需要一個溫馨的「互動挑戰」來緩解僵局。請從以下清單中**隨機挑選一項**，並詳細說明如何執行，以針對性地化解本次矛盾：
            * **挑戰 A (情感表達)：** 讓使用者向對方說出三句具體的感謝或肯定對方優點的話。
            * **挑戰 B (肢體暖心)：** 讓使用者給予對方一個溫暖的、長度超過五秒的擁抱。
            * **挑戰 C (共識重建)：** 邀請使用者與對方共同完成一件 15 分鐘內的簡單家務，並在過程中不談論矛盾。
    ...
    const requestBody = {
        ...
        generationConfig: { 
            temperature: 0.8 // 提高到 0.8，確保 AI 能夠根據清單進行創新選擇
        }
    };
