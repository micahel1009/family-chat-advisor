...
// ⭐ 強力除菌器：清除所有隱形字元與中文字，避免 ISO-8859-1 報錯
        ...
        snapshot.docChanges().forEach(change => {
            if (change.type === 'added') {
                ...
                if (!displayedMessageIds.has(change.doc.id)) {
                    ...
                    if (msg.text.includes("我希望破冰") && msg.senderId === sessionId) {
                        pledgeCount++; if (pledgeCount >= 2) triggerSuccessAI();
                    }
...
async function checkAndTriggerAI(lastText, senderName) {
    ...
    const triggers = ["煩", "吵架", "兇", "不聽話", "亂花錢", "錢", "未來", "尊重", "算了", "累了"];
...
// ⭐ 最終修正：網址改為 v1beta，這是目前 Flash 模型最穩定的接通網址！
async function triggerAIPrompt(mode, lastText, senderName) {
    ...
    let prompt = mode === "summary" ? `你現在是資深調解員。請用溫暖口吻總結衝突並建議輸入破冰句並加標籤 [TRIGGER_PLEDGE]：\n${hist}` : `你現在是溝通翻譯官。上下文：\n${hist}\n將這句翻譯成「背後的善意需求」：${senderName}: "${lastText}"`;
    try {
